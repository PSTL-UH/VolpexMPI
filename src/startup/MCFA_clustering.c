...
int* MCFA_sortedlist (int ** clusters, int nclusters, int *numelements, int redundancy)
{
	...
	if(numelementsleft[i]>=SL_numprocs/redundancy){
		...
		newnodes[num]=clusters[i][j];
		clusters[i][j]=-1;
		num++;
	    }
	    numelementsleft[i] = numelements[i] - SL_numprocs/redundancy;
	}
...
int** MCFA_sort_clusters(int *numelements, int **clusters, int nclusters)
{
	for(i=0;i<nclusters;i++)
		for(j=0;j<nclusters;j++){
			if (numelements[j]<numelements[j+1])
				temp = numelements[j];
				numelements[j] = numelements[j+1];
				numelements[j+1] = temp;
...
//	MCFA_create_subtree(subdistmat, redundancy, newnodes);
/*
    MCFA_node **subtree;
...
/*
			...
			if(coretree[j].right < 0 && rightflag == 0 ){
				leftflag = 0;
				j++;
			}
				
		if (mpitree[i].right >=0)
			if(coretree[j].left >= 0 && leftflag == 0){
                                mapnodes[mpitree[i].right] = coretree[j].left;
