...
//        printf("executable\t\t\t");
        ...
        while(curr !=NULL)
        {
                ...
                {
                        if (curr -> hostdata.id[i].jobID==id)
                        {
      ...
      //                          printf("\t%s\t",curr -> hostdata.executable);
...
//        printf("executable\t\t\t");
        ...
        {
                printf("%s\t\t",node->hostname);
                printf("   %d\t\t",node->numofProcs);
                MCFA_printf("    %d\t",node->lastPortUsed);
                for(i=0;i<node->numofProcs;i++){
                        printf("%d ",node->id[i].procID);
			printf("%s, ",node->id[i].exec);
		}
                printf("%d\t",node->status);
        }
	free(node);
...
int MCFA_add_hostname(struct MCFA_host_node **hostList, char *hostname,int numofProcs,int lastPortUsed, int status )
{
   ...
   //     strcpy(newhost.executable,executable);
...
int MCFA_printProclist(struct MCFA_proc_node *procList)
{
	...
	printf("Executable\n");
		...
		printf("%s\t  ",curr->procdata.fullrank);
		printf("%s\n  ",curr->procdata.executable);
                curr = curr->next;
                printf("\n");
        }
        return MCFA_SUCCESS;
...
int MCFA_print_proc(struct MCFA_process *proc)
{
	...
	printf("%s\t\t  %d\t\t  %d\t\t %d\t\t %s\t %s\n",proc->hostname,proc->portnumber,proc->sock,
...
/*    while(NULL != currproc)
    {
        if(id == currproc->procdata.id && currproc->procdata.status != 0)
            return 1;
