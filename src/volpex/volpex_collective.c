...
void VolPEx_reduce_ll(void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op, int root, MPI_Comm comm, int tag)
{
    MPI_Status *status = (MPI_Status *)MPI_STATUS_IGNORE;
    int j=0, k=0;
...
int VolPEx_Reduce(void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op, int root, MPI_Comm comm)
{
...
int VolPEx_Allreduce(void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op, MPI_Comm comm)
{
    int root = 0;
...
int  VolPEx_Alltoallv(void *sendbuf, int *sendcount, int *sdispls, MPI_Datatype sendtype,
                      void *recvbuf, int *recvcount, int *rdispls, MPI_Datatype recvtype, MPI_Comm comm)

{
    ...
    if( sendtype == recvtype  ) {
		for ( i=0; i < hdata[comm].mysize; i++){
		    if ( hdata[comm].myrank == i){
				memcpy(rbuf+rdispls[i]*sizeof(recvtype), sbuf+sdispls[i]*sizeof(sendtype), sendcount[i]*sizeof(sendtype));
				req[2*i]   = MPI_REQUEST_NULL;
				req[2*i+1] = MPI_REQUEST_NULL;
		    }
		    else {
			/* TBD: check for return code. What happens if an Irecv or Isend returns
			   ' no target available ?' we should probably abort the alltoall in 
			   that case as well. */
	            VolPEx_Irecv(rbuf+rdispls[i]*sizeof(recvtype), recvcount[i], recvtype, i, ALLTOALL_TAG, comm, &req[2*i]);
				VolPEx_Isend(sbuf+sdispls[i]*sizeof(sendtype), sendcount[i], sendtype, i, ALLTOALL_TAG, comm, &req[2*i+1]);
		    }
	    }
		waitcount = 2*hdata[comm].mysize;
		VolPEx_Waitall(waitcount, req, stats);
